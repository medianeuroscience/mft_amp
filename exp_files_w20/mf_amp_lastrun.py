...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/jacobtfisher/Dropbox/projects/inprogress/mf_amp/exp_files_w20/mf_amp_lastrun.py',
...
for thisAMP_prac in AMP_pracs:
    ...
    # keep track of which components have finished
    AMP_primeComponents = [AMP_wordstim, AMP_maskstim, AMP_key_reminder_1]
    ...
    while continueRoutine:
        ...
        if AMP_wordstim.status == NOT_STARTED and frameN >= 0:
            # keep track of start time/frame for later
            AMP_wordstim.frameNStart = frameN  # exact frame index
            AMP_wordstim.tStart = t  # local t and not account for scr refresh
            AMP_wordstim.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(AMP_wordstim, 'tStartRefresh')  # time at next scr refresh
            AMP_wordstim.setAutoDraw(True)
        if AMP_wordstim.status == STARTED:
            if frameN >= (AMP_wordstim.frameNStart + prime_dur):
                # keep track of stop time/frame for later
                AMP_wordstim.tStop = t  # not accounting for scr refresh
                AMP_wordstim.frameNStop = frameN  # exact frame index
                win.timeOnFlip(AMP_wordstim, 'tStopRefresh')  # time at next scr refresh
                AMP_wordstim.setAutoDraw(False)
        
        # *AMP_maskstim* updates
        if AMP_maskstim.status == NOT_STARTED and frameN >= prime_dur:
            # keep track of start time/frame for later
            AMP_maskstim.frameNStart = frameN  # exact frame index
            AMP_maskstim.tStart = t  # local t and not account for scr refresh
            AMP_maskstim.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(AMP_maskstim, 'tStartRefresh')  # time at next scr refresh
            AMP_maskstim.setAutoDraw(True)
        if AMP_maskstim.status == STARTED:
            if frameN >= (AMP_maskstim.frameNStart + mask_dur):
                # keep track of stop time/frame for later
                AMP_maskstim.tStop = t  # not accounting for scr refresh
                AMP_maskstim.frameNStop = frameN  # exact frame index
                win.timeOnFlip(AMP_maskstim, 'tStopRefresh')  # time at next scr refresh
    ...
    for thisComponent in AMP_primeComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    AMP_pracs.addData('AMP_wordstim.started', AMP_wordstim.tStartRefresh)
    AMP_pracs.addData('AMP_wordstim.stopped', AMP_wordstim.tStopRefresh)
    AMP_pracs.addData('AMP_maskstim.started', AMP_maskstim.tStartRefresh)
    AMP_pracs.addData('AMP_maskstim.stopped', AMP_maskstim.tStopRefresh)
...
for thisTrials_loop in trials_loop:
    ...
    for thisAMP_trial in AMP_trials:
        ...
        for thisComponent in AMP_primeComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        AMP_trials.addData('AMP_wordstim.started', AMP_wordstim.tStartRefresh)
        AMP_trials.addData('AMP_wordstim.stopped', AMP_wordstim.tStopRefresh)
        AMP_trials.addData('AMP_maskstim.started', AMP_maskstim.tStartRefresh)
        AMP_trials.addData('AMP_maskstim.stopped', AMP_maskstim.tStopRefresh)
