...
instr_AMP1_1 = visual.TextStim(win=win, name='instr_AMP1_1',
    text='Welcome to the second task! In this task, you will see the same symbols you just rated.',
    font='Verdana',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0, 
    color='#212121', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
...
# Initialize components for Routine "LDT_trial_3"
LDT_trial_3Clock = core.Clock()
...
for thisLDT_trial in LDT_trials:
    ...
    # ------Prepare to start Routine "LDT_trial_3"-------
    t = 0
    LDT_trial_3Clock.reset()  # clock
    frameN = -1
    continueRoutine = True
    # update component parameters for each repeat
    ...
    ldt_dur = random.randint(1, 6)
    # keep track of which components have finished
    LDT_trial_3Components = [LDT_word_trial, LDT_mask_trial, resp_LDT_trial, key_reminder]
    for thisComponent in LDT_trial_3Components:
        if hasattr(thisComponent, 'status'):
            thisComponent.status = NOT_STARTED
    
    # -------Start Routine "LDT_trial_3"-------
    while continueRoutine:
        # get current time
        t = LDT_trial_3Clock.getTime()
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
    ...
    thisExp.addData('corr', resp_LDT_prac.corr)
    thisExp.addData('exp_time', globalClock.getTime())
    # the Routine "LDT_trial_3" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
...
for thisTrial in trials:
    ...
    trials.addData('resp_memtask.keys',resp_memtask.keys)
    trials.addData('resp_memtask.corr', resp_memtask.corr)
    if resp_memtask.keys != None:  # we had a response
        trials.addData('resp_memtask.rt', resp_memtask.rt)
